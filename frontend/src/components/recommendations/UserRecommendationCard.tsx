...
export default function UserRecommendationCard({ user }: Props) {
  ...
  return (
    <div className="bg-[#1e1e1e] rounded-xl border border-white/10 p-3 hover:border-white/20 transition-all duration-200">
      <Link to={`/profile/${user.id}`} className="flex items-center gap-2 mb-2">
        <div className="w-7 h-7 rounded-full bg-[#8A9A5B]/20 flex items-center justify-center shrink-0">
          <span className="text-xs font-semibold text-[#8A9A5B]">
            {user.username[0].toUpperCase()}
          </span>
        </div>
        <span className="text-sm font-semibold text-white/90 hover:text-[#8A9A5B] transition-colors truncate">
          {user.username}
        </span>
      </Link>
      ...
      <button
        ...
        className="mt-2 w-full flex items-center gap-1.5 text-[11px] text-white/30 hover:text-[#8A9A5B] transition-colors py-1"
      >
        <Sparkles size={11} />
